...
## 2026-08-31 仓库面板说明与操作按钮统一

- 用户反馈仓库下拉框只有一个选项且按钮风格不一致。
- 已明确界面语义：下拉框只展示已经绑定到 RepoOps 的仓库；当前显示 `已连接 1 个` 是实际绑定数量，不代表 GitHub 账号只有一个仓库。
- 仓库辅助说明已改为提示“下拉框只显示已绑定仓库；点击‘绑定新仓库’可添加其他 GitHub 项目”，降低首次使用时的理解成本。
- “绑定新仓库”和“同步仓库数据”已统一为浅蓝色 iOS 风格描边按钮，并保留移动端满宽布局。
- 回归验证：前端 Vitest `19 passed`、Vite production build 通过、`git diff --check` 通过，前端首页 HTTP 200。

### 当前下一步

1. 刷新 `http://localhost:5174/`，点击“绑定新仓库”。
2. 从 GitHub 可访问但尚未绑定的仓库中选择一个并绑定；成功后下拉框应显示 2 个已连接仓库。
3. 若列表仍为空，检查 GitHub 账号是否确实拥有其他仓库以及 OAuth 授权范围；这不属于前端下拉框故障。
