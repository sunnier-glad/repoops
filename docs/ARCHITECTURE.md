...
- AI Release Notes 润色以当前草稿为基准生成独立 `AiAnalysis` 记录，保存原文、建议内容、变更摘要和模型结果；草稿变更后旧建议失效，只有用户手动载入编辑区并保存才会影响草稿。
- 任务只传递内部 ID，Worker 从数据库读取数据；这样不会把 access token 放入 Redis 消息。

## 当前边界

第一版不自动修复代码、不自动提交、不自动合并、不自动发布。AI 的输出是 PR 摘要、CI 失败解释、Release Notes 草稿和可审阅的润色建议，用户可以继续编辑和确认。
