...
import { bindRepository, getAvailableRepositories, getSession } from './api/client'
...
const qualityCards = [
  { label: 'CI 失败', value: '0', hint: '最近 24 小时', tone: 'danger', action: '查看失败任务' },
  { label: 'PR 协作', value: '0', hint: '待处理请求', tone: 'accent', action: '查看开放 PR' },
  { label: 'Release 质量', value: '—', hint: '最近版本', tone: 'success', action: '查看发布记录' },
]
    ...
    <main class="main-content">
      ...
      <section class="workspace-banner">
        <div>
          ...
          <p v-else-if="boundRepository">仓库已绑定，等待 GitHub 推送 PR、CI 和 Release 事件。</p>
      ...
      <section v-if="session" id="repository-panel" class="repository-panel" aria-label="仓库接入">
        ...
        <div v-if="boundRepository" class="repository-bound-state">
          ...
          <span>{{ boundRepository.webhook_configured ? 'Webhook 已配置，等待事件' : 'Webhook 配置待完成' }}</span>
        </div>
      ...
      <section class="quality-grid" aria-label="质量指标">
        <article v-for="card in qualityCards" :key="card.label" class="quality-card" :class="card.tone">
      ...
      <section class="lower-grid">
        <article class="panel activity-panel">
          <div class="panel-heading"><div><span class="eyebrow">EVENT STREAM</span><h2>最近活动</h2></div><button class="text-button" type="button">刷新 ↻</button></div>
          <div class="empty-state"><div class="empty-icon">⌁</div><strong>还没有 GitHub 事件</strong><p>{{ boundRepository ? '仓库已接入，push、PR、CI 和 release 会自动出现在这里。' : '先选择并绑定一个仓库，再接收 push、PR、CI 和 release。' }}</p><button class="empty-link" type="button" @click="boundRepository ? loadRepositories() : document.getElementById('repository-panel')?.scrollIntoView({ behavior: 'smooth' })">{{ boundRepository ? '刷新仓库' : '选择仓库' }} <span>↗</span></button></div>
        </article>
